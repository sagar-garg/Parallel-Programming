...
enum Platform_state_Attributes {
    Platform_state_UIRECT_POS = 0, Platform_state_UIRECT_C, Platform_state_UIRECT_ATTR_COUNT,
};

// Names for all uniforms
enum Platform_state_Uniforms {
    Platform_state_UIRECT_ORIGIN, Platform_state_UIRECT_SCALE, Platform_state_UIRECT_SCALE_P,
...
typedef struct Platform_state {
    ...
    float* buf_uirect_pos_data;
    int buf_uirect_pos_size, buf_uirect_pos_capacity;
    float* buf_uirect_c_data;
    int buf_uirect_c_size, buf_uirect_c_capacity;
...
void platform_redraw(double t) {
    global_platform.redraw_next = fmin(global_platform.redraw_next, t);
}
...
void _platform_handle_resize(int64_t width, int64_t height) {
    ...
    global_platform.ox = -5.f;
    global_platform.oy = -5.f;
    global_platform.sc = fminf((float)global_platform.screen_w / 1010.f, (float)global_platform.screen_h / 1010.f);
    global_platform.oy -=  (float)global_platform.screen_h / global_platform.sc - 1010.f;
    global_platform.ox -= ((float)global_platform.screen_w / global_platform.sc - 1010.f) * 0.5f;
...
void _platform_init(Platform_state* context) {
    ...
    char shader_f_uirect[] =
        ...
        "void main() {\n"
        "    float f = length(v_pos - v_c.xy) - v_c.z;\n"
        "    float t = clamp(f*scale_p + 0.5, 0.0, 1.0);\n"
        "    if (t >= 1.0) discard;\n"
        "    gl_FragColor = vec4(0, 0, 0, 1.0-t);\n"
        "}\n";

    GLuint program;
    OBST_PROGRAM_INIT(uirect);
    OBST_ATTRIB(UIRECT_POS, pos);
    OBST_ATTRIB(UIRECT_C, c);
    OBST_PROGRAM_LINK(uirect);
    OBST_UNIFORM(UIRECT_ORIGIN, origin);
    OBST_UNIFORM(UIRECT_SCALE, scale);
    OBST_UNIFORM(UIRECT_SCALE_P, scale_p);
...
void draw_circle(Platform_state* context, float x, float y, float r, float scale_p) {
    context->buf_uirect_pos_size += 12;
    if (context->buf_uirect_pos_size > context->buf_uirect_pos_capacity) {
        context->buf_uirect_pos_capacity = 2*context->buf_uirect_pos_size + 16;
        context->buf_uirect_pos_data = realloc(context->buf_uirect_pos_data,
            context->buf_uirect_pos_capacity * sizeof(float));
    }
    context->buf_uirect_c_size += 18;
    if (context->buf_uirect_c_size > context->buf_uirect_c_capacity) {
        context->buf_uirect_c_capacity = 2*context->buf_uirect_c_size + 16;
        context->buf_uirect_c_data = realloc(context->buf_uirect_c_data,
            context->buf_uirect_c_capacity * sizeof(float));
    }

    float* f = &context->buf_uirect_c_data[context->buf_uirect_c_size-18];
    for (int i = 0; i < 6; ++i) {
        *f++ = x;
        *f++ = y;
        *f++ = r;
    ...
    f = &context->buf_uirect_pos_data[context->buf_uirect_pos_size-12];
    *f++ = x-r - 1.f/scale_p; *f++ = y-r - 1.f/scale_p;
    *f++ = x+r + 1.f/scale_p; *f++ = y+r + 1.f/scale_p;
    *f++ = x-r - 1.f/scale_p; *f++ = y+r + 1.f/scale_p;
    *f++ = x-r - 1.f/scale_p; *f++ = y-r - 1.f/scale_p;
    *f++ = x+r + 1.f/scale_p; *f++ = y-r - 1.f/scale_p;
...
void _platform_draw(Platform_state* context, float* circles, int circles_count) {
    context->buf_uirect_pos_size = 0;
    context->buf_uirect_c_size = 0;

    glClear(GL_COLOR_BUFFER_BIT);
    glClear(GL_DEPTH_BUFFER_BIT);
    
    for (int i = 0; i < circles_count; ++i) {
        draw_circle(context, circles[3*i], circles[3*i+1], circles[3*i+2], global_platform.sc);
    }
    ...
    OBST_DO_UNIFORM(UIRECT_SCALE_P, 1f, global_platform.sc);

    OBST_DO_BUFFER(uirect, UIRECT_POS,    buf_uirect_pos,    2, GL_FLOAT, 0);
    OBST_DO_BUFFER(uirect, UIRECT_C,      buf_uirect_c,      3, GL_FLOAT, 0);

    glDrawArrays(GL_TRIANGLES, 0, context->buf_uirect_pos_size / 2);
...
void gui_create_window(int argc, char** argv) {
    ...
    linux_set_wm_prop(display, window, "WM_NAME", "nbody simulation");
    linux_set_wm_prop(display, window, "WM_ICON_NAME", "nbody");
...
void gui_draw(float* circles, int circles_count) {
    ...
    while (true) {
        ...
        if (redraw) {
            ...
            _platform_draw(&global_platform, circles, circles_count);
            break;
        }
